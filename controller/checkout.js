const getProducts = (req, res, db) => {
    ...
    db.select("counting as size","Products.price","checked","product_name as name", "store_id", "Products.id")
    .table("Cart")
    ...
    .where({"Cart.user_id":req.user.id}).andWhere('checked', 'TRUE')
    .then((result) => {
  ...
  const getUserInfo = (req, res, db) => {
    db.select("User.id", "User.name", "email", "phone", "street", "ward", "district", "city")
    .table("User")
